...
class PVGG16SeparateHP(PNetSeparateHP):
    def __init__(self, backbone, build_graph=False, random_init=False, ff_multiplier=(0.35,0.4), fb_multiplier=(0.25,0.0), er_multiplier=(0.01,0.01)):
        ...
        pmodule = ConvTranspose2d(512, 512, kernel_size=(5, 5), stride=(1, 1), padding=(2, 2))
        ...
        def fw_hook1(m, m_in, m_out):
            ...
            return e[0]
        self.backbone.features_compress[28].register_forward_hook(hook_fn)
        self.backbone.features_compress[31].register_forward_hook(fw_hook1)

        # PCoder number 2                                                                     \                                                                                            

        pmodule = Sequential(ConvTranspose2d(512, 512, kernel_size=(10, 10), stride=(2, 2), padding=(4, 4)), ReLU(inplace=True))
        ...
        def fw_hook2(m, m_in, m_out):
            ...
            return e[0]
        self.backbone.features_compress[41].register_forward_hook(fw_hook2)

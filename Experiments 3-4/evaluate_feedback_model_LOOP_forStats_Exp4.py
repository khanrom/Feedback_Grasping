...
from grasping_pvgg16_feedback_conn_3_2PC_spec_targ_fixed_dist_PC2_fb_even_longer_bb import PVGG16SeparateHP as PVGG16
import toml
...
def load_model(backbone_ckpt, pcoder1_ckpt, pcoder2_ckpt, device):
    model = VGG16Baseline()
...
def evaluate():
    ...
    model = load_model(args.PRETRAINED_MODEL, checkpoint_dir+args.PCODER1_CKPT, checkpoint_dir+args.PCODER2_CKPT,  device)
    print(model)
    # Load the test data
...
#            print(f'Average test loss for noise level {gauss_noise_level}: {average_losses}')
       ...
       #     json.dump( final_losses, json_file, indent=4)
        ...
        with open('test_losses_DIFF_conn_3_2PC_spec_targ_fixed_dist_PC2_fb_even_longer_bb.json', 'w') as json_file:

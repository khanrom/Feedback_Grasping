...
def pcoder_str(pcoder_idx, target_module_name, module_name, src_shape, target_shape, has_feedback, same_param, gscale, module=None, indent=""):
    ...
    if pcoder_idx > 1:
        target = f"self.pcoder{pcoder_idx-1}.rep"  
    else:
        # dict(backbone.named_children()).get(layer_name)
        if target_module_name[0] != "input_mem":
            target = f"self.target_layer"
        elif target_module_name[0] == "input_mem":
            target = "self.input_mem"
    ...
    if pcoder_idx == 1:
        s += indent + f"self.target_layer = None" + os.linesep
        if target_module_name[0] != "input_mem":
            s += indent + f"def hook_fn(module, input, output):" + os.linesep
            s += indent + f"    self.target_layer = output" + os.linesep
    ...
    if target_module_name[0] == "input_mem":
        s += indent + f"    self.target_layer = self.input_mem" + os.linesep
    s += indent + f"    return e[0]" + os.linesep
    if pcoder_idx == 1:
        if target_module_name[0] != "input_mem":
            s += indent + f"self.backbone.{target_module_name[0]}.register_forward_hook(hook_fn)" + os.linesep
...
def parse_toml(config_file):
    ...
    return name, layers, pmodules, target_layer, hyperparams, p_imports, input_size, gscale, same_hp
...
def predify_core(net, name, layers, pmodules, target_layer, hps, p_imports, input_size, gscale, same_hp, output_address):
    net.eval()
    ...
    if target_layer == [] or target_layer == None:
        raise Exception(f"Target layer is None")
    elif target_layer[0] == "input_mem":
        target_layer = "input_mem"
    else:
        target_layer = eval(f"net.{target_layer[0]}")
        target_layer.register_forward_hook(get_size)
    ...
    with open(output_address, "w") as f:
        ...
        if target_layer == None:
            raise Exception(f"Target layer is None")
        elif target_layer == "input_mem":
            target_shape = input_size
        else:
            target_shape = layer_sizes[target_layer]
        ...
        for idx, (module_name, module) in enumerate(zip(layers, modules), 1):
            ...
            s = pcoder_str(idx, target_module_name, module_name, src_shape, target_shape, idx < len(layers), same_hp, gscale, module=pmodules[idx-1], indent=indent)
            f.write(s)
            f.write(os.linesep)

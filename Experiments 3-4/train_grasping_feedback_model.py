...
from grasping_pvgg16_feedback_conn_3_2PC_spec_targ_even_longer_bb import PVGG16SeparateHP as PVGG16
...
wandb.init(
    ...
    config={
        ...
        "WEIGHT_DECAY": args.WEIGHT_DECAY,
        "FEEDBACK_CONNECTIVITY": 'conn_3_2PC_spec_targ_fixedPC1dist_even_longer_bb'
    }
)
...
def train_pcoders(net, epoch, train_loader, verbose=True):
    ...
    for batch_index, (images, _, _) in enumerate(train_loader):
        ...
        for i in range(NUMBER_OF_PCODERS):
            if i == 0:
                a = loss_function(net.pcoder1.prd, net.target_layer)
                loss = a
            else:
...
def test_pcoders(net, epoch, test_loader, verbose=True):
    ...
    for batch_index, (images, _, _) in enumerate(test_loader):
        ...
        for i in range(NUMBER_OF_PCODERS):
            if i == 0:
                final_loss[i] += loss_function(net.pcoder1.prd, net.target_layer).item()
            else:
                pcoder_pre = getattr(net, f"pcoder{i}")
                pcoder_curr = getattr(net, f"pcoder{i + 1}")
